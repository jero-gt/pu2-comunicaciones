...
[X_s,f] = fft_kit(x,fs);
stemCompleto([-fs/2 fs/2 0 abs(max(X_s))+10], 'f [KHz]', 'Amplitud', 'Espectro de la señal DBL.wav', 20, 'r.', 0.5, f, abs(X_s));
...
thetas_neg = thetas_pos.* (-1);

plot_comp_theta("y[n]|_{\theta = \alpha}",thetas_pos, fp, n, Ts, y);
plot_comp_theta("y[n]|_{\theta = \alpha}",thetas_neg, fp, n, Ts, y);
%%
